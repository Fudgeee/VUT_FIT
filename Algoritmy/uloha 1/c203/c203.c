...
*/
	if (q == NULL){		// ak je fronta prazdna hodi error
		queueError(QERR_INIT);
		return;
	}
	for (int i = 0;i < QUEUE_SIZE; i++)	// pre kazdy prvok v poli nastavi '*'
		q->arr[i] = '*';
	q->f_index = 0;		// zaciatok fronty 0
	q->b_index = 0;		// prve volne miesto 0
}
...
*/
	return (q->f_index == q->b_index); 	// ak ukazuju oba indexy na 1 prvok fronta je prazdna
}
...
*/
	if (queueEmpty(q)){	// skontroluje ci je fronta prazdna
		queueError(QERR_FRONT);
		return;
	}
	*c = q->arr[q->f_index];	// ak nie vrati znak zo zaciatku fronty
}
...
*/
	if (queueEmpty(q)){		// ak je fronta prazdna hodi error
		queueError(QERR_REMOVE);
		return;
	}
	q->f_index = nextIndex(q->f_index);	// index prveho prvku sa posunie na nasledujuci
}
