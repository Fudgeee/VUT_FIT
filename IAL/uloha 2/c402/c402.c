...
**/
	...
	if ((*RootPtr) == NULL) //ak je zoznam prazdny, tak vkladame koren 
	{
	...
	else
	{
		while (tmproot != NULL)
		{
			if (tmproot->Cont > Content) //ak je obsah mensi nez aktualny
			{
				if (tmproot->LPtr == NULL)
				{
					tBTNodePtr tmp = malloc(sizeof(struct tBTNode));	
					tmp->Cont = Content;
 					tmp->LPtr = NULL;
					tmp->RPtr = NULL;
					tmproot->LPtr = tmp; //vlozenie listu
					tmproot = tmproot->LPtr;
				}
				else
					tmproot = tmproot->LPtr;
			}
			else if (tmproot->Cont < Content) //ak je obsah vacsi nez aktualny
			{
				if (tmproot->RPtr == NULL)
				{
					tBTNodePtr tmp = malloc(sizeof(struct tBTNode));	
					tmp->Cont = Content;
 					tmp->LPtr = NULL;
					tmp->RPtr = NULL;
					tmproot->RPtr = tmp; //vlozenie listu
...
**/
	tStackP s1;
	SInitP(&s1);
	Leftmost_Inorder(RootPtr, &s1);
	while (!SEmptyP(&s1))
	{
		RootPtr = STopPopP(&s1);
		BTWorkOut(RootPtr);
		Leftmost_Inorder(RootPtr->RPtr, &s1);
	}
}
